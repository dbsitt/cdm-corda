...
abstract class BaseEventTestGJ(val samplesDirectory: String = "jsons") {
    ...
    protected fun sendNewTradeInAndCheckAssertionsGJ(jsonFileName: String) {
        //val newTradeEvent = readEventFromJson("/${samplesDirectory}/$jsonFileName")
        val jsonText = readTextFromFile("/${samplesDirectory}/$jsonFileName")
        //val future = node2.services.startFlow(TestFlowInitiating(newTradeEvent)).resultFuture
        val future = node2.services.startFlow(ExecutionFlow(jsonText)).resultFuture
