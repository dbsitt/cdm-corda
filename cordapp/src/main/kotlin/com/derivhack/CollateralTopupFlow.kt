...
@InitiatingFlow
@StartableByRPC
class CollateralTopupFlow(val partyName: String) : FlowLogic<SignedTransaction>() {
    ...
    override fun call(): SignedTransaction {
        ...
        val inputMoney = generateMoney(BigDecimal.valueOf(1000000))
        val inputParty = generateParty(partyName)
        ...
        val moneyState = CollateralWalletState (newMoneyJson, newPartyJson, walletReference, inputParty.meta.globalKey, inputParty.name.value, inputParty.partyId[0].value, BigDecimal.ZERO, participants, UniqueIdentifier())
...
@InitiatedBy(CollateralTopupFlow::class)
class CollateralWalletResponder(val flowSession: FlowSession) : FlowLogic<SignedTransaction>() {
