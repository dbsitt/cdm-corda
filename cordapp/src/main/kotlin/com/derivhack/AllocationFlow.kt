...
import net.corda.cdmsupport.functions.AgentHolder.Factory.partyMap
...
//        val evt = parseEventFromJson(allocationJson)
        val request = parseAllocationRequestFromJson(allocationJson)
        val statesAndRef = serviceHub.vaultService.queryBy<ExecutionState>().states
        val stateAndRef = statesAndRef.first { it.state.data.execution().meta.globalKey == request.executionRef }

        val state = stateAndRef.state.data

        val evt = allocationBuilderFromExecution(BigDecimal.valueOf(request.amount1),
                BigDecimal.valueOf(request.amount2),
                partyMap[request.subAccount1]!!, partyMap[request.subAccount2]!!, state)
